...
typedef struct SSPMState {
    ...
    uint64_t addr, ctrl_addr;
    char *boot_layout;
    bool analysis;
    uint8_t initial[BANK_SIZE], bytes[BANK_SIZE];
    uint32_t tx_pending, rx_pending;
} SSPMState;

static uint64_t sspm_data_read(void *opaque, hwaddr offset, unsigned size)
{
    SSPMState *s = opaque;
    uint32_t value = 0;
    for (unsigned i = 0; i < size; i++) {
        value |= (uint32_t)s->bytes[offset + i] << (8 * i);
    }
    return value;
}

static void sspm_data_write(void *opaque, hwaddr offset, uint64_t value,
                            unsigned size)
{
    SSPMState *s = opaque;
    for (unsigned i = 0; i < size; i++) {
        s->bytes[offset + i] = value >> (8 * i);
    }
}
...
static const MemoryRegionOps sspm_data_ops = {
    .read = sspm_data_read, .write = sspm_data_write,
...
static void sspm_realize(DeviceState *dev, Error **errp)
{
    ...
    if (!sspm_mapping_ok(s->addr, BANK_SIZE, errp) ||
        !sspm_mapping_ok(s->ctrl_addr, 8, errp)) {
        return;
    }
    if (s->addr < s->ctrl_addr + 8 && s->ctrl_addr < s->addr + BANK_SIZE) {
    ...
    sspm_reset(dev);
    memory_region_init_io(&s->data, OBJECT(dev), &sspm_data_ops, s,
                         TYPE_SSPM "-data", BANK_SIZE);
